...
const ContentModeration = ({ showModal, onClose }) => {
  let { classroomState: { is_moderation_enabled },
    classroomActions: { saveContentModerationSettings }} = useClassroomContext();
  ...
  return <Modal title="Content Moderation Settings"
    visible={showModal}
    onOk={handleSave}
    onCancel={onClose}
    okText={'Save'}
  >
    <h3 level={4}>Uses Azure Cognitive Services to detect profanity
      and match against common blacklists.</h3>
    <br />
